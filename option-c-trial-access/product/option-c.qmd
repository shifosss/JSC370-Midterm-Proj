...
subtitle: "Analysis, exploratory results, and model-ready data products"
author: ""
date: today
format:
  html:
...
This section models county-level diabetes burden (`places_diabetes`) as a function
of county diabetes clinical trial access and county socioeconomic conditions,
following the modeling plan in `option_c_burden_target_modeling_plan.md`.
Three complementary models are estimated:

| Model | Purpose |
|-------|---------|
| **Elastic Net** | Linear baseline; regularization handles correlated SES predictors |
| **Random Forest** | Nonlinear ensemble; permutation importance ranks predictors without standardization |
| **XGBoost** | Gradient boosting; highest predictive performance; SHAP enables group-level attribution |
...
shap.summary_plot(shap_values, X_raw, feature_names=all_predictors, show=True)
```
...
Three complementary models have now estimated the relationship between county
diabetes burden (`places_diabetes`) and two groups of predictors:
**trial-access variables** (`log_trials`, `log_dist`) and
**socioeconomic / demographic variables**.

Key questions answered:

1. **Do trial-access predictors matter after controlling for SES?**
   - If Elastic Net coefficients for `log_trials` and `log_dist` are non-zero
     and the Random Forest / XGBoost permutation importance and SHAP values
     for those features are meaningful, trial geography is aligned with burden.
   - If they shrink to near zero while SES coefficients dominate, trial
     placement tracks structural advantage rather than disease burden.

2. **Which predictor group drives county diabetes burden most?**
   - The group-level SHAP bar chart (cell above) gives the definitive answer:
     whichever group has the highest mean |SHAP| is the dominant driver.

3. **Across-model consistency**
   - Agreement across Elastic Net, Random Forest, and XGBoost strengthens
     conclusions drawn from any single model.
   - If XGBoost R² substantially exceeds Elastic Net R², this suggests
     important nonlinear or interaction effects that the linear model misses.
